...
function atualizarPedidosRealizados() {
    ...
    clientes.forEach((clientePedido, index) => {
        ...
        li.innerHTML = `<strong>${clientePedido.nome}</strong>:<br>Total: R$ ${clientePedido.total.toFixed(2)}
                        ...
                        <button onclick="editarPedido(${index})">Editar Pedido</button>`; // Botão para editar o pedido
...
function editarPedido(index) {
    ...
    // Carregar o pedido existente para o pedido atual
    pedido = [...cliente.pedido]; // Copia os itens do pedido finalizado para o atual
    total = cliente.total; // Atualiza o total

    // Atualiza o formulário com o nome do cliente
    document.getElementById("cliente").value = cliente.nome;

    // Remove o pedido da lista para ser atualizado após edição
    clientes.splice(index, 1);

    atualizarPedido();
    salvarNoLocalStorage(); // Atualiza o LocalStorage com o pedido reaberto
}
